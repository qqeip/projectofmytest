...
type
  ...
  protected
    ...
    procedure BeforeDeleteObject(aObject: TRMView); override;
...
var
  RMGridReportDesignerClass: TClass;
...
constructor TRMGridReportPage.CreatePage(aParentReport: TRMReport; aSize, aWidth, aHeight, aBin: Integer; aOr: TRMPrinterOrientation);
begin
  inherited;
...
var
  ...
  begin
    lPageWidth := Self.PrinterInfo.PageWidth - Self.spMarginLeft - Self.spMarginRight;
    lBand := nil;
  ...
  var
    ...
    lSize: Integer;
  begin
    t := RMCreateObject(lCell.View.ObjectType, lCell.View.ClassName);
...
procedure TRMGridReportPage.BeforeDeleteObject(aObject: TRMView);
var
  i: Integer;
begin
  if FRowBands = nil then Exit;

  if aObject is TRMCustomBandView then
  begin
    for i := 1 to FRowBands.Count - 1 do
    begin
      if FRowBands[i] = aObject then
        FRowBands[i] := nil;
    end;
  end;
end;
...
procedure TRMGridReportPage.Loaded;
var
  i: Integer;
  t: TRMView;
begin
...
begin
  ...
  FReportFlagStr := 'ReportMachine GridReport File 3.0';
  DesignerClass := RMGridReportDesignerClass;
...
begin
  ...
  begin
    ...
    AddClass(cRM_Grid, TRMGridReportPage, 'TRMGridReportPage');
    AddClass(cRM_Grid, TRMGridReport, 'TRMGridReport');
