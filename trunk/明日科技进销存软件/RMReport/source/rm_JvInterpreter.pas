...
{$IFDEF COMPILER6_UP}
  Variants,
{$ENDIF COMPILER6_UP}
  rm_JvInterpreterParser, rm_JvComponent;
const
...
begin
  if DllInstance > 0 then
    Result := rm_JvInterpreter.CallDllIns(DllInstance, FunName, Args, FunDesc.FParamTypes,
      FunDesc.ResTyp)
  else
    Result := rm_JvInterpreter.CallDll(DllName, FunName, Args, FunDesc.FParamTypes,
      FunDesc.ResTyp)
end;
//=== TJvInterpreterEvent ====================================================
...
function TJvInterpreterAdapter.NewEvent(const aUnitName: string; const aFunctionName,
  aEventType: string; AOwner: TJvInterpreterExpression; AObject: TObject): TSimpleEvent;
var
...
begin
  ...
  begin
    ...
    if Cmp(JvInterpreterEventDesc.Identifier, aEventType) then
    begin
      Event := JvInterpreterEventDesc.EventClass.Create(AOwner, AObject, aUnitName, aFunctionName);
    ...
    begin
      case TTyp of
        ...
        ttRS:
          if TVarData(Result).VType = varEmpty then
          begin
            Result := 0;   // µ±[]·µ»ØÖµÎª0  whf add
            //ErrorExpected(LoadStr2(irExpression)); // whf delete
          end
          else
            Exit;
      else
...
var
  ...
  var
    FC: PFunContext;
    C: Integer;
    lVarList: TJvInterpreterVarList; // VARLEAKFIX
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //LEAKY:  TVarData(PFunContext(FunContext).LocalVars.FindVar('', 'Result').Value).VType := varEmpty;
        //LEAKY:  TVarData(PFunContext(FunContext).LocalVars.FindVar('', 'Result').Value).VPointer := nil;

        //VARLEAKFIX begin - Feb 2004 - Warren Postma. Fix suggested by ivan_ra@mail.ru
        lVarList := PFunContext(FunContext).LocalVars;
        lVarList.GetValue('Result', FVResult, Args);
        VarClear(lVarList.FindVar('', 'Result').Value);
        //VARLEAKFIX end.
      end;
      Args.Count := C;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ErrorExpected('''' + kwEND + '''')
    else if TTyp = ttDoubleQuote then
      NextToken
    else
      SkipStatement1;
    if TTyp = ttEnd then
