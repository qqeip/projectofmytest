...
type
  ...
  private
    ...
    function LocateBuilding(aBuildingName :String; aTree:TTreeView; aQueryType: TNodeType= BUILDING):boolean;
...
function TFm_MainForm.LocateBuilding(aBuildingName: String; aTree:TTreeView; aQueryType: TNodeType):boolean;
var
  lCurrNode_City, lCurrNode_Area, lCurrNode_Suburb,
  lCurrNode_BuildStatus, lCurrNode_Build,//lCurrNode_DRSOUTType
  lCurrNode_DRSType, lCurrNode_DRS: TTreeNode;
begin
  result:= false;
        ...
        begin
          ...
          begin
            case aQueryType of
              ...
              DRS:
              begin
                if not TNodeParam(lCurrNode_Build.Data).HaveExpanded then    //直放站类型节点
                  lCurrNode_Build.Expand(false);
                lCurrNode_DRSType:= lCurrNode_Build.getNextSibling;
                while (lCurrNode_DRSType<>nil) and (lCurrNode_DRSType.Data<>nil) do
                begin
                  if not TNodeParam(lCurrNode_DRSType.Data).HaveExpanded then
                    lCurrNode_DRSType.Expand(false);
                  lCurrNode_DRS:= lCurrNode_DRSType.getNextSibling;
                  while (lCurrNode_DRS<>nil) and (lCurrNode_DRS.Data<>nil) do
                  begin
                    if uppercase(lCurrNode_DRS.Text)=uppercase(aBuildingName) then
                    begin
                      aTree.Selected := lCurrNode_DRS;
                      result := true;
                      if result then
                        exit;;
                    end;
                    lCurrNode_DRS:= lCurrNode_DRS.getNextSibling;
                  end;
                end;
              end;
            end;
...
begin
   if not assigned(FormDRSInfoMgr) then
      begin
        FormDRSInfoMgr:=TFormDRSInfoMgr.Create(nil);
        AddToTab(FormDRSInfoMgr);
      end;
      SetTabIndex(FormDRSInfoMgr);
      FormDRSInfoMgr.WindowState:=wsMaximized;
      FormDRSInfoMgr.Show;
end;
...
begin
  ...
  begin
    if TreeViewAll.Visible then
      if not LocateBuilding(cxComboBoxSearch.Text,TreeViewAll,BUILDING) then
        Application.MessageBox('未找到室分点！','提示',MB_OK	);
    if TreeViewSub.Visible then
      if not LocateBuilding(cxComboBoxSearch.Text,TreeViewSub,BUILDING) then
        Application.MessageBox('未找到室分点！','提示',MB_OK	);
    if TreeViewDRS.Visible then
      if not LocateBuilding(cxComboBoxSearch.Text,TreeViewDRS,BUILDING) then
        Application.MessageBox('未找到室分点！','提示',MB_OK	);
  end;
  ...
  begin
    if TreeViewAll.Visible then
      if TreeViewAll.Visible then
        if not LocateBuilding(cxComboBoxSearch.Text,TreeViewAll,DRS) then
          Application.MessageBox('未找到直放站！','提示',MB_OK	);
    if TreeViewDRS.Visible then
      if TreeViewDRS.Visible then
        if not LocateBuilding(cxComboBoxSearch.Text,TreeViewDRS,DRS) then
          Application.MessageBox('未找到直放站！','提示',MB_OK	);
  end;
end;
