...
uses
  ...
  StringUtils, DBClient, IniFiles, Menus, ExtCtrls, dxSkinsCore;
...
begin
  ...
  SpeedButtonSearchClick(Sender);
end;

function TFormAlarmContentModule.GetBlurQueryWhere(aFieldName, aFieldCode,
...
begin
  ...
  try
    ...
    lSqlStr:= 'select * from drs_alarm_content where 1=1 ' +
              lWhereStr +
...
begin
  ...
  try
    ...
    begin
      ...
      Lsqlstr:='select ALARMCONTENTCODE,ALARMCONTENTNAME from drs_alarm_content'+
               ' order by alarmcontentcode' ;
