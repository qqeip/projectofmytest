...
uses
  ...
  UnitUserMgr in 'UnitUserMgr.pas' {FormUserMgr},
  UnitDllCommon in '..\..\框架文件\UnitDllCommon.pas',
  UnitModifyPwd in 'UnitModifyPwd.pas' {FrmModifyPwd};
...
{$R *.res}
  ...
  begin
    Result:= nil;
    if Assigned(FormUserMgr) then Exit;
    gTempInterface:=TempInterface;
    gPublicParam:=PublicParam;
    gDllMsgCall := @DllMessageCall;
    FormUserMgr:=TFormUserMgr.create(Application);
    Result:=FormUserMgr;
  end;

  procedure CloseForm;stdcall;
  begin
    if Assigned(FormUserMgr) then
    begin
      FreeAndNil(FormUserMgr);
    end;
  end;
