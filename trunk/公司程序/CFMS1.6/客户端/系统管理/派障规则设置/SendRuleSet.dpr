...
uses
  ...
  Classes,
  Forms,
  UnitSendRuleSet in 'UnitSendRuleSet.pas' {FormSendRuleSet},
...
{$R *.res}
 ...
 function CallDll(Application:TApplication; TempInterface:IDataModuleRemoteDisp; PublicParam: TPublicParameter;DllMessageCall: TDllMessage):TForm;
  begin
    Result:= nil;
    if Assigned(FormSendRuleSet) then Exit;
    gTempInterface:=TempInterface;
    gPublicParam:= PublicParam;
    gDllMsgCall := @DllMessageCall;
    FormSendRuleSet:=TFormSendRuleSet.create(Application);
    Result:=FormSendRuleSet;
  end;

  procedure CloseForm;stdcall;
  begin
    if Assigned(FormSendRuleSet) then
    begin
      FreeAndNil(FormSendRuleSet);
    end;
  end;
