...
uses
  ...
  Classes,
  Forms,
  UnitRoleMgr in 'UnitRoleMgr.pas' {FormRoleMgr},
...
{$R *.res}
  ...
  begin
    Result:= nil;
    if Assigned(FormRoleMgr) then Exit;
    gTempInterface:=TempInterface;
    gPublicParam:= PublicParam;
    gDllMsgCall := @DllMessageCall;
    FormRoleMgr:=TFormRoleMgr.create(Application);
    Result:=FormRoleMgr;
  end;

  procedure CloseForm;stdcall;
  begin
    if Assigned(FormRoleMgr) then
    begin
      FreeAndNil(FormRoleMgr);
    end;
  end;
