...
begin
  ...
  try
    ...
    liPrinterInfo := AllocMem(ByteCnt);
    try
      EnumPrinters(PRINTER_ENUM_DEFAULT, nil, 5, liPrinterInfo, ByteCnt, ByteCnt, StructCnt);
      if StructCnt > 0 then
        lDevice := liPrinterInfo.pPrinterName
      else
      begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Orientation = rmpoPortrait then
        begin
          p.PageWidth := RMToScreenPixels(PaperWidths[lindex] * 100, rmutMMThousandths);
          p.PageHeight := RMToScreenPixels(PaperHeights[lindex] * 100, rmutMMThousandths);
        end
        else
        begin
          p.PageWidth := RMToScreenPixels(PaperHeights[lindex] * 100, rmutMMThousandths);
          p.PageHeight := RMToScreenPixels(PaperWidths[lindex] * 100, rmutMMThousandths);
        end;
      end;
    end
    else
    begin
      ...
      p.PageWidth := RMToScreenPixels(PaperWidth * 100, rmutMMThousandths);
      p.PageHeight := RMToScreenPixels(PaperHeight * 100, rmutMMThousandths);
    ...
    FFactorX := p.PrinterPageWidth / p.PageWidth;
    FFactorY := p.PrinterPageHeight / p.PageHeight;
  finally
    FLock.Release;
  end;
end;
...
begin
  ...
  try
    ...
    try
      ...
      lPDevMode^.dmFields := DM_COPIES or DM_ORIENTATION or DM_PAPERSIZE or
        DM_DUPLEX or DM_COLOR or DM_DUPLEX;
