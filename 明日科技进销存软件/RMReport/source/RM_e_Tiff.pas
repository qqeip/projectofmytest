...
begin
  ...
  RMRegisterExportFilter(Self, 'Tiff Picture' {RMLoadStr(SBMPFile)}, '*.tif');
end;
...
begin
  ...
  try
    ...
    FImageEnIO.SaveToStreamTIFF(Stream);
    FImageEnIO.AttachedBitmap := nil;
    {$ELSE}
    WriteTiffToStream(Stream, lBitmap);
    {$ENDIF}
  finally
    lBitmap.Free;
  end;
...
begin
  ...
  try
    if FPageNo = 0 then
      Stream := TFileStream.Create(lFileName, fmCreate)
    else
