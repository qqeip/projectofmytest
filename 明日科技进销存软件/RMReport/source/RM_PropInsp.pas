...
{$IFDEF Delphi6}, Types, Variants{$ENDIF};
...
begin
  if (InplaceEditor <> nil) and (InplaceEditor is TELPropsPageInplaceEdit) then
    TELPropsPageInplaceEdit(InplaceEditor).CloseUp(False);
end;
...
procedure TELCustomPropsPage.CMMouseLeave(var Msg: TMessage);
begin
  inherited;
...
procedure TELCustomPropsPage.WMMouseMove(var Msg: TWMMouseMove);
begin
  inherited;
...
procedure TELCustomPropsPage.SetCellHints(Value: Boolean);
begin
...
procedure TELCustomPropsPage.DoHint(X, Y: Integer);
var
  ...
  function _GetText(aCol, aRow: Integer): string;
  var
    LItem: TELPropsPageItem;
  begin
    Result := '';
    ...
    if LItem <> nil then
    begin
      LIdent := 3 + (12 + LItem.Ident) * Ord(ACol = 0);
      case aCol of
        0:
          begin
            if RMLocalizedPropertyNames then
              Result := LItem.VirtualCaption
            else
              Result := LItem.Caption;
          end;
        1: Result := LItem.DisplayValue;
      end;
    end
    else
      LIdent := 3 + 12 * Ord(ACol = 0); ;
  end;
...
begin
  ...
  begin
    liText := _GetText(LogCol, LogRow);
    Canvas.Font := Font;
    if (liText <> '') and (Canvas.TextWidth(liText) + LIdent > ColWidths[LogCol]) and
      not (csDesigning in ComponentState) then
    begin
      FHintWnd.Color := Application.HintColor;
      R := FHintWnd.CalcHintRect(Screen.Width div 2, liText, FHintWnd);
      ...
      if not IsWindowVisible(FHintWnd.Handle) or not ((R.Left = OldR.Left) and (R.Top = OldR.Top)) then
        FHintWnd.ActivateHint(R, liText);
    end
...
begin
  inherited;
  Canvas.Font := Font;
  DefaultRowHeight := Canvas.TextHeight('Wg') + 5;
end;
...
begin
  ...
  FBitmap.Width := 8;
  FBitmap.Height := 8;
  FBitmap.Canvas.Brush.Color := FBitmapBkColor;
  FBitmap.Canvas.Brush.Style := bsSolid;
  FBitmap.Canvas.FillRect(Rect(0, 0, FBitmap.Width, FBitmap.Height));
  LI := 0;
  while LI < Width do
  begin
    FBitmap.Canvas.Pixels[LI, 0] := clBtnShadow;
    Inc(LI, 2);
  end;
...
begin
  ...
  begin
    ...
    ListVisible := True;
    Invalidate;
    Windows.SetFocus(ActiveList.Handle);
...
begin
  ...
  begin
    ...
    if FCaption <> '' then
    begin
      FVirtualCaption := FCaption;
      FPropCommon := FCaption;
      for i := 0 to FPropInfoList.Count - 1 do
      begin
        lPropInfo := FPropInfoList[i];
        if AnsiCompareText(lPropInfo.PropTrueName, FCaption) = 0 then
        begin
          FVirtualCaption := lPropInfo.PropName;
          if lPropInfo.PropCommon <> '' then
            FPropCommon := lPropInfo.PropCommon
          else
            FPropCommon := FVirtualCaption;
          Break;
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    if praMethodProp in LAttrs then
    begin
      EditButtonClick;
    end
    else if (praValueList in LAttrs) and not (praDialog in LAttrs) then
    begin
...
begin
  ...
  begin
    if Assigned(TELPropertyInspector(Owner).OnBeforeModify) then
      TELPropertyInspector(Owner).OnBeforeModify(Self, Self.Caption);
...
var
  ...
  LFilterRes: Boolean;
begin
  if (pkEvents in PropKinds) and Assigned(OnGetEditorClass) then
  begin
    Result := nil;
...
var
  liItem: TELPropsPageItem;
begin
  liItem := GetActiveItem;
  UpdateItems;
  if Assigned(FOnAfterModify) then FOnAfterModify(Self, liItem.Caption, liItem.DisplayValue);
end;
