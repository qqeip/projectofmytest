...
uses
  ...
  , rm_JvComCtrls, rm_JvPanel, rm_JvTabControl
  {$ELSE}
  , JvComCtrls, JvPanel, JvTabControl
  {$ENDIF}
  {$ENDIF}
  ;
const
...
type
  ...
  {$IFDEF JVCLCTLS}
  TRMPanel = TJvPanel;
  TRMPageControl = TJvPageControl;
  TRMTabControl = class(TJvTabControl)
    {$ELSE}
  TRMPanel = TPanel;
  TRMPageControl = TPageControl;
...
var
  ...
  procedure RestoreToolbarPosition(t: TRMToolbar);
  var
    Ini: TRegIniFile;
    X, Y: Integer;
    DN: string;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if t.Owner <> nil then
        begin
          NewDock := t.Owner.FindComponent(DN) as TRMDock;
          if NewDock <> nil then
          begin
            {$IFDEF USE_TB2K}
            t.CurrentDock := NewDock;
            {$ELSE}
            t.DockedTo := NewDock;
            {$ENDIF}
            t.DockPos := X;
