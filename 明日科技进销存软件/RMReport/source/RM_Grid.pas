...
uses Messages, Windows, SysUtils, Classes, Graphics, Menus, Controls, Forms,
  StdCtrls, Mask, Clipbrd, RM_Class;
...
type
  ...
  TRMBeforeChangeCell = procedure(aGrid: TRMGridEx; aCell: TRMCellInfo) of object;
  ...
  public
    ...
    procedure ReCreateView(ObjectType: Byte; const ClassName: string);
  published
  ...
  private
    ...
    FOnRowHeaderClick: TNotifyEvent;
    FOnRowHeaderDblClick: TNotifyEvent;
    FOnBeginSizingCell: TNotifyEvent;
    FOnBeforeChangeCell: TRMBeforeChangeCell;
  ...
  public
    ...
    property OnBeforeChangeCell: TRMBeforeChangeCell read FOnBeforeChangeCell write FOnBeforeChangeCell;
  published
    ...
    property OnRowHeaderClick: TNotifyEvent read FOnRowHeaderClick write FOnRowHeaderClick;
...
procedure TRMCellInfo.ReCreateView(ObjectType: Byte; const ClassName: string);
var
  t: TRMView;
begin
  if (FView.ObjectType <> rmgtAddIn) and (FView.ObjectType = ObjectType) then Exit;
  if (FView.ObjectType = rmgtAddIn) and (AnsiCompareText(FView.ClassName, ClassName) = 0) then Exit;

  t := RMCreateObject(ObjectType, ClassName);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        _SelectCells;
        if Assigned(FOnRowHeaderClick) then FOnRowHeaderClick(Self);
      end;
    end;
  end
  ...
  case FGridState of
    rmgsSelecting, rmgsColMoving, rmgsRowMoving:
      begin
        ...
        // 向左上方拖动选择并超出显示范围
        if (CellHit.X < LeftCol) and (LeftCol > FixedCols) then
          _ScrollGrid;
        if (CellHit.Y < TopRow) and (TopRow > FixedRows) then
          _ScrollGrid;

        if (CellHit.X >= FFixedCols) and (CellHit.Y >= FFixedRows) and
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        State := FGridState;
      if State = rmgsRowSizing then
        Cur := Screen.Cursors[crVSplit]
      else if State = rmgsColSizing then
...
begin
  ...
  for lCol := aRect.Left to aRect.Right do
    RMWriteInt32(aStream, ColWidths[lCol]);
  for lRow := aRect.Top to aRect.Bottom do
    RMWriteInt32(aStream, RowHeights[lRow]);
...
begin
  ...
  lXOffset := aRect.Left - RMReadInt32(aStream);
  lYOffset := aRect.Top - RMReadInt32(aStream);
...
begin
  FInLoadSaveMode := True;
  try
    lSaveWidth := ColWidths[0];
    ClearGrid;
    ColWidths[0] := lSaveWidth;

    RMReadWord(aStream);
    ColCount := RMReadInt32(aStream);
    RowCount := RMReadInt32(aStream);
    for liCol := 1 to ColCount - 1 do
      ColWidths[liCol] := RMReadInt32(aStream);
    for liRow := 1 to RowCount - 1 do
      RowHeights[liRow] := RMReadInt32(aStream);
    ...
    begin
      ...
      begin
        ...
        begin
          ReadCellFromBuffer(liCell, aStream, 0, 0);
        end;
      end;
    end;
  finally
  ...
  begin
    ...
    begin
      if (i <> aCell.StartCol) or (j <> aCell.StartRow) then
        Cells[i, j].Assign(aCell); //FromCell(liCell);
    end;
  end;
...
var
  ...
  lCell: TRMCellInfo;
begin
  RMWriteWord(aStream, GridVersion);
