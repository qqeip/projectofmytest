...
unit rm_JvInterpreterFm;

interface

uses
  SysUtils, Classes, Controls, Forms,
  rm_JvInterpreter, rm_JvUtils;
...
uses
  TypInfo,
  rm_JvInterpreterConst, rm_JvStrUtil;
          ...
          begin
            ...
            begin
              SetMethodProp(Com, PropList^[I],
                ...
                PChar(FMethodList[F]){PString(FMethodList[F])^}, PropList^[I]^.PropType^.Name, Self)));
            end;
          end;
        end;
    finally
...
var
  ...
  // whf add,修复不能调入form中过程的bug
  lOldObj: TObject;
  lOldInstance: TObject;
  lOldObjTyp: Word;
...
begin
  ...
  if (Args.Obj = nil) and (lOldObj <> nil) and (lOldObjTyp = varObject) and
    (lOldObj is TJvInterpreterForm) then // whf add 2004.07.19
  begin
    lOldInstance := CurInstance;
    CurInstance := lOldObj;
    try
      Result := Result or inherited GetValue(Identifier, Value, Args);
    finally
      CurInstance := lOldInstance;
    end;  
  end
  else
    Result := Result or inherited GetValue(Identifier, Value, Args);
