...
type
  ...
  private
    FReport: TRMReport;
    FReportDesigner: TRMCustomDesigner;
  protected
  public
  ...
  published
    ...
    property ReportDesigner: TRMCustomDesigner read FReportDesigner write FReportDesigner;
  end;
...
uses RMD_ExpFrm, RM_Utils, RM_Const, RMD_DlgSelectReportType;
...
begin
  ...
  begin
    ...
    FReportDesigner := TRMReportItem(Source).ReportDesigner;
  end
  else
...
begin
  ...
  begin
    ...
    else if aComponent is TRMCustomDesigner then
    begin
      for i := 0 to FReports.Count - 1 do
      begin
        if FReports[i].ReportDesigner = aComponent then
          FReports[i].ReportDesigner := nil;
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    if (FReports[0].Report <> nil) and (FReports[0].ReportDesigner <> nil) then
    begin
      FCurrentReport := FReports[0].Report;
      FReports[0].ReportDesigner.OnSaveReport := OnSaveReportEvent;
    end;
  end
  else
  begin
    ...
    try
      ...
      begin
        ...
        if (FReports[i].Report <> nil) and (FReports[i].ReportDesigner <> nil) then
        begin
          FCurrentReport := FReports[i].Report;
          FReports[i].ReportDesigner.OnSaveReport := OnSaveReportEvent;
        end;
      end;
    finally
      tmp.Free;
...
begin
  ...
  begin
    ...
    if (liIndex >= 0) and (FReports[liIndex].ReportDesigner <> nil) then
    begin
      ...
      FReports[liIndex].ReportDesigner.OnSaveReport := OnSaveReportEvent;
      FCurrentReport.DesignReport;
      if FRefreshFlag then
