...
uses
  ...
  Classes,
  Forms,
  UnitAlarmContentMgr in 'UnitAlarmContentMgr.pas' {FormAlarmContentMgr},
...
{$R *.res}
 ...
 function CallDll(Application:TApplication; TempInterface:IDataModuleRemoteDisp; PublicParam: TPublicParameter;DllMessageCall: TDllMessage):TForm;
  begin
    Result:= nil;
    if Assigned(FormAlarmContentMgr) then Exit;
    gTempInterface:=TempInterface;
    gPublicParam:= PublicParam;
    gDllMsgCall := @DllMessageCall;
    FormAlarmContentMgr:=TFormAlarmContentMgr.create(Application);
    Result:=FormAlarmContentMgr;
  end;

  procedure CloseForm;stdcall;
  begin
    if Assigned(FormAlarmContentMgr) then
    begin
      FreeAndNil(FormAlarmContentMgr);
    end;
  end;
