...
uses
  ...
  Classes,
  Forms,
  UnitUserFieldSet in 'UnitUserFieldSet.pas' {FormUserFieldSet},
  ...
  UnitColComentCommon in '..\..\框架文件\UnitColComentCommon.pas';
...
{$R *.res}
  ...
  begin
    Result:= nil;
    if Assigned(FormUserFieldSet) then Exit;
    gTempInterface:=TempInterface;
    gPublicParam:= PublicParam;
    gDllMsgCall := @DllMessageCall;
    FormUserFieldSet:=TFormUserFieldSet.create(Application);
    Result:=FormUserFieldSet;
  end;

  procedure CloseForm;stdcall;
  begin
    if Assigned(FormUserFieldSet) then
    begin
      FreeAndNil(FormUserFieldSet);
    end;
  end;
