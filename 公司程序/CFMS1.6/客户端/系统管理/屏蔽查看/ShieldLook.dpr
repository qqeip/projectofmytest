...
uses
  ...
  Classes,
  Forms,
  UnitShieldLook in 'UnitShieldLook.pas' {FormShieldLook},
...
{$R *.res}
 ...
 function CallDll(Application:TApplication; TempInterface:IDataModuleRemoteDisp; PublicParam: TPublicParameter;DllMessageCall: TDllMessage):TForm;
  begin
    Result:= nil;
    if Assigned(FormShieldLook) then Exit;
    gTempInterface:=TempInterface;
    gPublicParam:= PublicParam;
    gDllMsgCall := @DllMessageCall;
    FormShieldLook:=TFormShieldLook.create(Application);
    Result:=FormShieldLook;
  end;

  procedure CloseForm;stdcall;
  begin
    if Assigned(FormShieldLook) then
    begin
      FreeAndNil(FormShieldLook);
    end;
  end;
